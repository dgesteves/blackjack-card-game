...
describe('Game services', () => {
  ...
  it('creates a new game successfully', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${BASE_URL}/start`, {
      method: 'POST',
    });
  });
  ...
  it('performs a hit action successfully', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${BASE_URL}/hit/${gameId}`, {
      method: 'POST',
    });
  });
  ...
  it('performs a stand action successfully', async () => {
    ...
    expect(result).toEqual(mockGameState);
    expect(mockFetch).toHaveBeenCalledWith(`${BASE_URL}/stand/${gameId}`, {
      method: 'POST',
    });
  });
